...
{
    //return sprintf here('You are trying with username=>%s, password=>%s', $username, $password);
    $pdo = Database::getInstance()->getConnection();
    //check the existence for these
    ...
    if ($user_set->fetchColumn() > 0) {
        ...
        while ($founduser = $user_match->fetch(PDO::FETCH_ASSOC)) {
            //this is where we begin setting the sessions
            $id = $founduser['user_id'];
            $_SESSION['user_id'] = $id;
            $_SESSION['user_name'] = $founduser['user_name'];

            //this is where we need to update the user table and set the user_ip column to $ip
